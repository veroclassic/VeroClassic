...
*Setup instructions for a Gitian build of Veroclassic using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Veroclassic
...
- [Building Veroclassic](#building-veroclassic)
...
Clone the git repositories for veroclassic and Gitian and then checkout the veroclassic version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the veroclassic repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for Gitian to work offline.

Building Veroclassic
----------------

To build Veroclassic (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the veroclassic repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/veroclassic/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/veroclassic-project/veroclassic
...
For example:
```bash
URL=https://github.com/user/veroclassic.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit veroclassic=${COMMIT} --url veroclassic=${URL} ../veroclassic/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit veroclassic=${COMMIT} --url veroclassic=${URL} ../veroclassic/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit veroclassic=${COMMIT} --url veroclassic=${URL} ../veroclassic/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/veroclassic-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/veroclassic-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/veroclassic-build.assert
```
...
[veroclassic/gitian.sigs](https://github.com/veroclassic-project/gitian.sigs.ver/) repository, or if that's not possible create a pull
request. 
